...
def jointPlotter(numLayers, params_pre, params_post, plot_hist = True):
    ...
    if (plot_hist == True):
        ...
        for i in range(numLayers):
            ...
            interMax = np.maximum(np.max(w1), np.max(w2))
            interMin = np.minimum(np.min(w1), np.min(w2))
...
#            sns.distplot(w2, rug=False, kde=False, norm_hist=False, label = "Weights after train")
            ...
            interMax = np.maximum(np.max(b1), np.max(b2))
            interMin = np.minimum(np.min(b1), np.min(b2))
